...
//			 thisDto.setCreateUserName(user.getUserName());
	 ...
	 * @throws Exception
	 */
	public void save(MyUserPrincipal myUserPrincipal, ManifestSaveUpdateDto wsuDto, CommonsMultipartFile file) throws Exception {
		try {
...
//			 manifest.setCreateUser(session.getAttribute("username").toString());
	 ...
	 * @throws Exception
	 */
	public void update(MyUserPrincipal myUserPrincipal,ManifestSaveUpdateDto wsuDto, CommonsMultipartFile file) throws Exception {
		try {
			...
			currWb.setUpdateUserId(createuserId);
			dao.update(currWb);
